...
object BootstrapAlerts extends Factory with Loggable {
  ...
  def render = {
    ...
    def computeMessageDiv(
      args: (List[(NodeSeq, Box[String])], NoticeType.Value)): NodeSeq =
      args match {
        case (messages, noticeType) =>
          ...
          notices(messages).toList match {
            ...
            case msgs if (msgs.length == 1) =>
              <div id={ noticeType.id } class={
                "alert-message %s".format(lowerCaseTitle(noticeType))
              } data-alert="">
                <a class="close" href="#">&times;</a>
                <p>{
                  noticeTitle(noticeType).map(
                    t => <strong>{ t }</strong>).
                    openOr(Text(""))
                } { msgs(0) }</p>
              </div>
